...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const results = query(emailExist, [email]);
        ...
        const insertResult = query(sql, [nome1, nome2, email, hashedPassword, telefone, cod_adm]);
...
router.put('/:id', verificationAdmUser, async (req, res) => {
    ...
    try {
        const results = query(sql, [nome1, nome2, email, senha, telefone, id_usuario, req.query.id_adm]);
...
router.delete('/:id', verificationAdmUser, async (req, res) => {
    ...
    try {
        const results = query(sql, [id_usuario, id_adm]);
...
router.get('/', verificationAdmUser, async (req, res) => {
    ...
    try {
        const results = query(sql, [id_adm]);
        res.json(results);
    } catch (err) {
